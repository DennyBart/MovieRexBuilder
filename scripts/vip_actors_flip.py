...
DATABASE_URL = 'mysql://dennyb:p0llyw4nt5a5nacker@dennyb.mysql.pythonanywhere-services.com/dennyb$movie_recs'
...
if __name__ == "__main__":
    ...
    with open("actors.vip", "r") as f:
        names_list = [line.strip() for line in f.readlines()]

    # Update the VIP status
    not_found = update_vip_status(names_list, cast_type='ACTOR')

    # Output the list of names not found
    if not_found:
        with open("actors_not_found.txt", "w") as f:
